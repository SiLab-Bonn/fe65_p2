...
#     else:
    try:
        popt, _ = curve_fit(zcurve, PlsrDAC, zcurve_data, p0=[repeat_command, 33, np.std(zcurve_data)], check_finite=False)
...
#     print "threshold of fit", popt[1], " tdac value ", tdac_val
    return [popt[0], popt[1], popt[2], chi2, tdac_val, en]
    ...
    if abs(max_occ) <= 1e-08 and not vth1:  # or index == 0: occupancy is zero or close to zero
        ...
        try:
            if vth1:
                ...
                popt, _ = curve_fit(scurve, PlsrDAC, scurve_data, p0=[repeat_command, mu_guess, 0.01], check_finite=False)
