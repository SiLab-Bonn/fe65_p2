''' Helper script to start scans
'''

from fe65p2.scans.noise_scan import NoiseTuning
...
logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s - %(filename)s - [%(levelname)-8s] (%(threadName)-10s) %(message)s")
...
def timewalk_sc(noise_mask, th_mask, pix_list, mu, sigma):
    ...
    step = 0.005
    logging.info('TIMEWALK mu, min, max, step: %f %f %f %f',
                 mu, range_min, range_max, step)
    ...
    time_sc.tdc_table(
        len(np.arange(scanrange[0], scanrange[1], scanrange[2])) + 2)
    time_sc.dut.close()
...
class status_sc(ScanBase):
    ...
    def load_bit(self, bit):
        if(bit == True):
            self.dut['intf']._sidev.DownloadXilinx(
                fe65p2_path + "/firmware/ise/fe65p2_mio.bit")
...
if __name__ == "__main__":
    ...
    for c in cycle(range(0, 2)):  # goes on forever
        ...
        for i in range(1, 9):  # 1 to 9 for all columns
            ...
            timewalk_sc(thrs_mask, thrs_mask, pixels,
                        musigma['mu'], musigma['sigma'])
